...
// Helper: Render the promo section with live counter
...
export const root = new Elysia()
  ...
  .get("/", async ({ jwt, redirect, cookie: { auth, jobId } }) => {
    // Render the promo section ALWAYS, before auth check
    // (all visitors get this, not just logged-in users)

    // Optionally: also render login/upload/history if user is authenticated
    // If you want to show uploads for authenticated users only, keep this block below

    let user: ({ id: string } & JWTPayloadSpec) | false = false;
    let mainUi = null;

    // You may choose to skip all of this if you don't want uploads for non-auth users
    if (!ALLOW_UNAUTHENTICATED) {
      if (FIRST_RUN) return redirect(`${WEBROOT}/setup`, 302);
      if (!auth?.value) {
        // Show just the promo/counter
        return (
    ...
    if (ALLOW_UNAUTHENTICATED) {
      ...
      user = { id: newUserId };
      if (!auth) {
        // Still render promo/counter even if no cookie
        return (
          <BaseHtml webroot={WEBROOT}>
            <>
              <Header
      ...
      if (
        ...
        if (!existingUser) {
          ...
          // Show promo/counter if login fails
          return (
            <BaseHtml webroot={WEBROOT}>
              <>
                <Header
    ...
    if (!user) {
      // Fallback: show promo/counter for non-logged in users
      return (
        <BaseHtml webroot={WEBROOT}>
          <>
            <Header
    ...
    // --- If user is logged in, show the upload/convert UI plus the promo/counter ---
    // (or adjust to allow unauthenticated upload if desired)
    // create a new job
        ...
        <>
          ...
          <main class={`w-full flex-1 px-2 sm:px-4`}>
            ...
            <article class="article">
              ...
              <div
                id="dropzone"
                class={`relative flex h-48 w-full items-center justify-center rounded border border-dashed
            ...
            >
              ...
              <article class="article w-full">
                ...
                <div class="select_container relative">
                  <article
                    class={`convert_to_popup absolute z-2 m-0 hidden h-[30vh] max-h-[50vh] w-full flex-col
                      ...
                      >
                        ...
                        <ul class="convert_to_target flex flex-row flex-wrap gap-1">
                          {targets.map((target) => (
                            <button
                              tabindex={0}
                              class={`target rounded bg-neutral-700 p-1 text-base
              ...
              <input
                class={`w-full btn-primary opacity-100
                  disabled:cursor-not-allowed disabled:opacity-50`}
                type="submit"
                value="Convert"
                disabled
              />
              {/* Insert the PromoSection here as well */}
              {PromoSection()}
            </form>
          </main>
          <script src="script.js" defer />
